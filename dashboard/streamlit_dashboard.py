...
def alerts_over_time_chart(data: pd.DataFrame):
...
def count_of_alerts(data: pd.DataFrame):
...
def temperature_over_time_chart(data: pd.DataFrame):
...
def moisture_over_time_chart(data: pd.DataFrame):
...
def most_alerted_botanist_chart(data: pd.DataFrame):

    chart = alt.Chart(data).mark_bar().encode(
        alt.X('botanist_name'),
        alt.Y('alert count'),
        alt.Color('botanist_name', legend=None)
    )
    return chart
