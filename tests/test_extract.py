...
def test_save_to_json_contents_correct(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr('etl.extract.OUTPUT_FILE', str(fake_output))
...
def test_check_new_endpoints_new_found(monkeypatch):
    ...
    monkeypatch.setattr('etl.extract.fetch_data_by_id',
...
def test_extract_data(monkeypatch, tmp_path):
    ...
    fake_output = tmp_path/'plant_data_raw_test.json'
    monkeypatch.setattr('etl.extract.OUTPUT_FILE', str(fake_output))
    monkeypatch.setattr('etl.extract.BASE_NUM_ENDPOINTS', 3)
    monkeypatch.setattr('etl.extract.multiprocessing.Pool',
                        lambda *a, **k: FakePool())
    monkeypatch.setattr('etl.extract.check_new_endpoints', lambda: 3)
    monkeypatch.setattr('etl.extract.fetch_data_by_id',
...
def test_extract_data_some_errors(monkeypatch, tmp_path):
    ...
    fake_output = tmp_path/'plant_data_raw_test.json'
    monkeypatch.setattr('etl.extract.OUTPUT_FILE', str(fake_output))
    monkeypatch.setattr('etl.extract.BASE_NUM_ENDPOINTS', 7)
    monkeypatch.setattr('etl.extract.multiprocessing.Pool',
                        lambda *a, **k: FakePool())
    monkeypatch.setattr('etl.extract.check_new_endpoints', lambda: 7)
    monkeypatch.setattr('etl.extract.fetch_data_by_id',
